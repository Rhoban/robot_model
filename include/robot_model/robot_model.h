...
public:
  ...
  bool hasDof(std::string name);

  // Return all DOF names
  std::vector<std::string> getDofNames();

  // Sets the value of a dof [rad]
  void setDof(std::string name, double value);

  // Reset all DOFs to 0 position
  void resetDofs();

  // Gets the id of a RBDL body, raise exception if not found
  unsigned int getBodyId(std::string name);
  unsigned int getJointId(std::string name);

  // Gets the position transformation from frame "src" to frame "dest"
  Eigen::Vector3d position(std::string srcFrame, std::string dstFrame,
                           const Eigen::Vector3d& point = Eigen::Vector3d::Zero());
  Eigen::Vector3d jointPosition(std::string jointName, std::string frame);
